...
1. __Time to first save __— How quickly new users save their first keyword
2. __Return visits __— Users coming back to their topical maps
3. __Keywords per project __— Average number of keywords saved per project
...
- Monitor usage patterns and API costs
...
This MVP is intentionally minimal: one user type, one core job\. The bet is that solopreneurs will find value in a tool that immediately shows them which keywords they can realistically win\. Speed and simplicity are the differentiators — not feature parity with enterprise tools\.
